const RealCoordinates = require("../models/realCoordinatesModel");
...
exports.assignCoordinates = async (req, res, next) => {
    ...
    try {
        ...
        // Create damage state
        const coordinates = await RealCoordinates.create(req.body);

        res.status(201).json({
            status: "real coordinates successfully added to component",
...
exports.updateCoordinates = async (req, res, next) => {
    ...
    try {
        ...
        const coordinates = await RealCoordinates.findByIdAndUpdate({ "_id": req.body._id },
            req.body, {
                new: true,
                runValidators: true,
        });
...
exports.deleteCoordinates = async (req, res, next) => {
    ...
    try {
        const coordinates = await RealCoordinates.findByIdAndDelete({ "_id": req.body._id });
